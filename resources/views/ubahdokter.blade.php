...
<body>
    ...
    <div class="col-lg-8 mt-3 mx-2 mt-0 bg-dark p-3 text-white">
        <form action="{{ route('patch.ubahdokter', ['id'=>$data_dokter->id]) }}" method="post">
            @csrf
            @method('patch')
            <h1 class="mt-3 col-lg-8">Update Data Dokter</h1>
            <div class="mb-3">
              <label for="name" class="form-label">Nama</label>
              <input type="text" name="name" class="form-control" id="name" value="{{ $data_dokter->name ?? old('name') }}">
                @error('name')
                    <p>{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-3">
              ...
              <input type="text" name="usia" class="form-control" id="usia" value="{{ $data_dokter->usia ?? old('usia') }}">
              @error('usia')
                    <p>{{ $message }}</p>
                @enderror
            </div>
