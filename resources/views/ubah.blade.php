...
<body>
    ...
    <div class="col-lg-8 mt-3 mx-2 mt-0 bg-dark p-3 text-white">
        <form action="{{ route('patch.ubah', ['id'=>$data_pasien->id]) }}" method="post">
            @csrf
            @method('patch')
            <h1 class="mt-3 col-lg-8">Edit Data</h1>
            <div class="mb-3">
              <label for="name" class="form-label">Nama</label>
              <input type="text" name="name" class="form-control" id="name" value="{{ $data_pasien->name ?? old('name') }}">
                @error('name')
                    <p>{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-3">
              ...
              <input type="text" name="usia" class="form-control" id="usia" value="{{ $data_pasien->usia ?? old('usia') }}">
              @error('usia')
                    <p>{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-3">
              <label for="text" class="form-label">Penyakit</label>
              <input type="text" name="penyakit" class="form-control" id="penyakit" value="{{ $data_pasien->penyakit ?? old('penyakit') }}">
              @error('penyakit')
                    <p>{{ $message }}</p>
                @enderror
            </div>
